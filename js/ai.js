...
const AI = (() => {
  ...
  const TUNE = {
    atkDmg:      2.3,   // 공격 점수 = 예상피해 * atkDmg - 비용 * atkCost
    atkCost:     1.6,
    finishBonus: 40,    // 상대를 탈락시킬 수 있을 때 가산
    healW:       1.0,   // 회복 카드 가중 (낮을수록 덜 회복 = 더 잘 죽음)
    reserveEarly:10,    // 7라운드 이하에서 남겨두려는 HP
    reserveLate:  7,    // 8라운드 이후
    reservePen:  1.9,   // 여유분 미달 1당 감점
    tieBreak:    20     // 10라운드 동점 회피 가중 (클수록 데스매치가 덜 발생)
  };
  /* 위 조합의 측정치: 1500게임 기준 데스매치 124 : 조기종료 355 ≒ 1 : 2.86
   * (test/tune.html 로 재측정 가능. tieBreak 를 낮추면 데스매치가 늘어납니다) */
